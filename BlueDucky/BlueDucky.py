...
class Adapter:
    ...
    def _run_command(self, command):
        ...
        # Replace Adapter.set_property with this version (keeps original semantics but handles ints)
        def set_property(self, prop, value):
            """
            Set property using hciconfig and verify the setting.
            Accepts ints for class (e.g. 0x002540 or 9536) or strings for name.
            """
            # Format value for hciconfig:
            # - If int, use decimal string (hciconfig commonly accepts decimal for class)
            # - If hex-like string (starts with 0x), convert to decimal
            if isinstance(value, int):
                value_str = str(value)
            else:
                value_str = str(value)
                # if they passed "0x...." convert to decimal to match earlier manual usage
            ...
            verify_command = ["hciconfig", self.iface]
            verification_result = run(verify_command)
            # For name, check value_str or the quoted name may appear; be lenient
            if isinstance(value, int):
                expected = value_str
            else:
                expected = value if isinstance(value, str) else value_str

            if expected not in verification_result.stdout and str(value_str) not in verification_result.stdout:
    ...
    def disable_pscan(self):
        try:
            pscan_command = ["sudo", "hciconfig", self.iface, "noscan"]
            pscan_result = run(pscan_command)
            if pscan_result.returncode != 0:
                log.error(f"Failed to disable PSCAN: {pscan_result.stderr}")
                raise ConnectionFailureException("Failed to disable PSCAN")
        except Exception as e:
            log.error(f"Error disabling PSCAN: {e}")
            raise
...
class L2CAPClient:
    ...
    def connect(self, timeout=None):
        ...
        try:
            ...
            if self.connected is False & self.port == 14:
                print(
                    '{reset}[{red}!{reset}] {red}CRITICAL ERROR{reset}: {reset}Attempted Connection to {red}{target_address} {reset}was {red}denied{reset}.')
...
def setup_bluetooth(target_address, adapter_id):
    """
    Robust automated adapter configuration flow:
      1. Stop bluetoothd (avoid fighting BlueZ while setting hciconfig options)
      2. Bring iface up
      3. Set class & name
      4. Start bluetoothd again so pydbus / BlueZ operations work
      5. Wait for adapter to be UP RUNNING and available on DBus
      6. Start register_hid_profile process and return Adapter instance
    """
    # stop any running daemon to avoid races (best-effort)
    try:
        log.info("Stopping bluetooth service to apply low-level settings...")
        run(["sudo", "systemctl", "stop", "bluetooth"], timeout=10)
    except Exception as e:
        log.warning(f"Could not stop bluetooth service (continuing): {e}")

    # ensure the adapter is up at kernel level
    try:
        log.info("Bringing adapter up...")
        run(["sudo", "hciconfig", adapter_id, "up"])
    except Exception as e:
        log.error(f"Failed to bring up {adapter_id}: {e}")
        raise ConnectionFailureException("Failed to bring adapter up")

    # verify adapter reports UP RUNNING (retry briefly)
    for _ in range(6):
        out = run(["hciconfig", "-a"]).stdout
        if adapter_id in out and "UP RUNNING" in out:
            break
        time.sleep(0.4)
    else:
        log.warning(f"{adapter_id} never reported UP RUNNING (continuing anyway).")

    # set class & name using values you prefer (class accepts decimal; we accept ints or hex)
    try:
        # prefer decimal for class; the Adapter.set_property will accept both ints & hex strings
        temp_adapter = Adapter(adapter_id)  # create a minimal adapter for dbus checks (may raise)
    except Exception:
        # Adapter DBus object may not be present until bluetoothd starts — that's ok
        temp_adapter = None

    try:
        log.info("Setting adapter class & name via hciconfig...")
        # use integers for class to avoid ambiguity; 0x002540 == decimal 9536
        class_value = 0x002540
        # call Adapter.set_property if possible (it will use hciconfig under the hood)
        if temp_adapter:
            temp_adapter.set_property("class", class_value)
            temp_adapter.set_property("name", "Robot POC")
        else:
            # fallback to direct commands
            run(["sudo", "hciconfig", adapter_id, "class", str(int(class_value))])
            run(["sudo", "hciconfig", adapter_id, "name", "Robot POC"])
    except Exception as e:
        log.warning(f"Failed to set class/name via Adapter helper: {e}. Trying direct hciconfig commands...")
        try:
            run(["sudo", "hciconfig", adapter_id, "class", str(int(0x002540))])
            run(["sudo", "hciconfig", adapter_id, "name", "Robot POC"])
        except Exception as e2:
            log.error(f"Failed to set adapter class/name: {e2}")
            raise ConnectionFailureException("Failed to configure adapter properties")

    time.sleep(0.25)

    # Now start bluetooth service again so BlueZ / DBus are available for HID profile registration
    try:
        log.info("Starting bluetooth service...")
        run(["sudo", "systemctl", "start", "bluetooth"], timeout=10)
    except Exception as e:
        log.warning(f"Could not start bluetooth service: {e}")

    # wait for bluetoothd to appear on DBus and for adapter to be present
    for i in range(12):
        try:
            adapter = Adapter(adapter_id)
            # small sanity check: adapter.Powered may exist
            try:
                _ = adapter.adapter
                break
            except Exception:
        ...
        # give a last chance: try to construct Adapter anyway
        try:
            adapter = Adapter(adapter_id)
        except Exception as e:
            log.error("Adapter not visible on DBus after starting bluetoothd: %s", e)
            raise ConnectionFailureException("Adapter not available on DBus")

    # Ensure the adapter is powered on via BlueZ (DBus)
    try:
        adapter.power(True)
    except Exception as e:
        log.warning(f"Could not set adapter Powered via DBus: {e} — continuing")

    # Now spawn the HID profile registration process (as you had previously)
    try:
        log.info("Starting HID profile registration process...")
        profile_proc = Process(target=register_hid_profile, args=(adapter_id, target_address))
        profile_proc.start()
        child_processes.append(profile_proc)
    except Exception as e:
        log.error("Failed to start register_hid_profile process: %s", e)
        # Not fatal here — but depends on your needs
        raise

    # re-verify adapter final state for debugging
    info = run(["hciconfig", adapter_id]).stdout
    log.info("Adapter state after automated setup:\n%s", info.strip())
